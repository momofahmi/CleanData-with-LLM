                ...
                for ref in refs:
                    ...
                    if (x.dropna() % 1 == 0).all():      # integer-valued column -> integer offset
                        ...
                        if _ks(a_ - di, b_) <= _ks(a_ - delta, b_) + 0.005:
                            delta = di
                    D0 = _ks(a_, b_)
                    D_add = _ks(a_ - delta, b_)
                    D_mul = _ks(a_ / ratio, b_) if ratio and np.isfinite(ratio) and ratio > 0 else 1.0
                    # noise floor: KS 95% critical value for two samples of these sizes
                    floor = 1.36 * np.sqrt((len(a_) + len(b_)) / (len(a_) * len(b_)))
                    aligned = lambda D: D <= max(0.25 * D0, 1.5 * floor) and D < 0.05
                    ...
                    if add_ok:
                        findings.append(dict(kind="additive_shift", slicer=s, level=lv, ref=ref, target=t, n=int(len(a_)), delta=round(delta, 4),
                                             evidence=f"KS {D0:.3f}->{D_add:.3f} after subtracting {delta:.4g}; consistent across {len(offs)} strata (sign {sign:.0%})",
                        ...
                        findings.append(dict(kind="multiplicative_shift", slicer=s, level=lv, ref=ref, target=t, n=int(len(a_)), ratio=round(ratio, 4),
                                             evidence=f"KS {D0:.3f}->{D_mul:.3f} after dividing by {ratio:.4g}",
